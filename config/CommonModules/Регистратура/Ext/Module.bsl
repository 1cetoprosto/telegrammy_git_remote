...
Процедура ОчисткаНапоминанийКонтрагентам() Экспорт
	...
	Запрос.Текст = 
		...
		|	РегистрСведений.НапоминанияКонтрагента КАК НапоминанияКонтрагента
		|ГДЕ
		|	НапоминанияКонтрагента.Доставлено = ИСТИНА";
...
Функция ПолучитьМастераПоЛогину(Логин) Экспорт 
	
	ТекущийМастер = Справочники.Мастера.ПустаяСсылка();
	
	Если ВРег(ПараметрыСеанса.ТекущийПользователь) = "ADMIN" Тогда
		ТекущийМастер = Справочники.Мастера.НайтиПоРеквизиту("Логин", "lyudmilabadyk");
	...
	Если ТекущийМастер = Справочники.Мастера.ПустаяСсылка() Тогда
		...
		Сообщение.Текст = "Пользователь " + ПараметрыСеанса.ТекущийПользователь + " не зарегистрирован. Обратитесь в техническую поддержку.";
		Сообщение.Сообщить(); 
	КонецЕсли;
