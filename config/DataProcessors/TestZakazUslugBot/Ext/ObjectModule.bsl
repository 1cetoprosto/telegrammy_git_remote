...
Процедура ВыборУслуг(Сообщение, стрОтвет = Неопределено)
	...
	Если ПредыдущийСообщение.Свойство("Мастер", Мастер) Тогда
		...
		Если Не стрОтвет = Неопределено Тогда
			...
			ИначеЕсли ПредыдущийСообщение.Свойство("ТекДата", ТекДата) Тогда
				Результат = Телеграм.editMessageText(Сообщение.Чат, Сообщение.message_id,, ТекстСообщения,,,ПолучитьКнопкиУслуги(Мастер)); //ПолучитьКнопкиКалендарь(ТекущаяДата(), Мастер)
			КонецЕсли;
		Иначе
			Если мНоменклатура.Количество() > 0 Тогда // есть выбранная номенклатура
				Результат = Телеграм.sendMessage(Сообщение.Чат, ТекстСообщения,,,,,ПолучитьКнопкиУслуги(Мастер, мНоменклатура)); //ПолучитьКнопкиКалендарь(ТекущаяДата(), Мастер)
			ИначеЕсли ПредыдущийСообщение.Свойство("ТекДата", ТекДата) Тогда
...
Процедура ПоказатьЗаписиКонтрагента(Сообщение)
	...
	ТекстСообщения = "Ваші записи:";
	
	Заказ = "";
	// пройдемся в цикле по заказам
	ТекстСообщения = ТекстСообщения + ПолучитьТекстЗаказа(Заказ);
...
Функция ПолучитьТекстЗаказа(Заказ)
	...
	ТекстСообщения = "<b>Ви замовили:</b>" + Символы.ПС;
	Для каждого строкаЗаказа Из Заказ.ВремяЗаписи Цикл
		ТекстСообщения = ТекстСообщения + строкаЗаказа.Номенклатура + " - " + строкаЗаказа.Цена +Символы.ПС;	
	КонецЦикла;
