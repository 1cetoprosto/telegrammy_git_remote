...
Процедура ПрочитатьИОбработатьЗапросБоту() Экспорт
	...
	//Если ЭтоМастер(ЗапросБоту) Тогда
	#Область Контакт
	...
	//ИначеЕсли ЗапросБоту.ТипОтвета = Перечисления.ТипыОтветовТелеграм.Кнопка Тогда
	...
	//		//Результат = Телеграм.sendMessage(ЗапросБоту.Чат, ТекстСообщения,,,,,ПолучитьКнопкиВремяМастера(ЗаписьНаДату));
	//		Результат = Телеграм.editMessageText(ЗапросБоту.Чат, ЗапросБоту.message_id,, ТекстСообщения,Перечисления.ВидФорматирования.HTML,,ПолучитьКнопкиВремяМастера(ЗаписьНаДату));
	//		
	//		СессияКонтрагента.Вставить("Мастер", 		Мастер);
	//		СессияКонтрагента.Свойство("ТекДата", 		ТекДата);
	//		Боты.ЗаписатьСессиюКонтрагента(Телеграм.Бот, ЗапросБоту.Чат, СессияКонтрагента);
	...
	//				Заказ = СоздатьЗаказ(ЗапросБоту.Чат, Результат);
	//			КонецЕсли;
	//			
	//			Если Заказ = Неопределено Тогда
