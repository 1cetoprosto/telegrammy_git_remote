...
Процедура ПрочитатьИОбработатьСообщение(Сообщение) Экспорт
	...
	Если Сообщение.ТипОтвета = Перечисления.ТипыОтветовТелеграм.Кнопка Тогда	
		...
		ИначеЕсли Лев(ДанныеСообщения.callback_query.data,6) = "ВРЕМЯ_" Тогда
			...
			КэшКонтрагента = Боты.ПолучитьОписаниеСессииКонтрагента(Телеграм.Бот, Сообщение.Чат);
			КэшКонтрагента.Свойство("Мастер", Мастер);
			
			ЗаписатьВремя(Сообщение, Заказ);
		...
		Иначе // была нажата кнопка, поищем её в ВариантахОтветов
			...
			Если стрОтвет.Операция = "СсылкаМастер" Тогда
				...
				КэшКонтрагента = Новый Структура;
				КэшКонтрагента.Вставить("Мастер", Мастер);
				Боты.СоздатьСессиюКонтрагента(Телеграм.Бот, Сообщение.Чат, КэшКонтрагента);
			...
			ИначеЕсли стрОтвет.Операция = "СсылкаНоменклатура" Тогда
				...
				Заказ = Неопределено;
				Если НЕ КэшКонтрагента.Свойство("Заказ", Заказ) Тогда // не нашли заказ, создадим
				...
				КэшКонтрагента = Новый Структура;
				КэшКонтрагента.Вставить("Заказ", Заказ);
				КэшКонтрагента.Вставить("Мастер", Мастер);
				Боты.СоздатьСессиюКонтрагента(Телеграм.Бот, Сообщение.Чат, КэшКонтрагента);
...
Процедура ПоказатьКалендарь(Сообщение, СмещениеКвоМесяцев)
	...
	КэшКонтрагента.Свойство("ТекДата", ТекДата);
	...
	КэшКонтрагента.Вставить("Мастер", Мастер);
	КэшКонтрагента.Вставить("Заказ", Заказ);
	КэшКонтрагента.Вставить("ТекДата", ТекДата);
	Боты.СоздатьСессиюКонтрагента(Телеграм.Бот, Сообщение.Чат, КэшКонтрагента);
...
Процедура ПоказатьВыборУслуг(Сообщение, стрОтвет = Неопределено)
	...
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		КэшКонтрагента.Вставить("Номенклатура", Номенклатура);	
	КонецЕсли;
	Боты.СоздатьСессиюКонтрагента(Телеграм.Бот, Сообщение.Чат, КэшКонтрагента);
...
Процедура ЗаписатьВремя(Сообщение, Заказ = Неопределено)
...
Функция ПолучитьТекстЗаказаПоМастеру(Заказ, Мастер)
	...
	Пока ВыборкаМастер.Следующий() Цикл
		...
		//Пока ВыборкаКонтрагент.Следующий() Цикл
			...
			ТекстСообщения = "<b>Новий запис:</b>" + Символы.ПС;  
