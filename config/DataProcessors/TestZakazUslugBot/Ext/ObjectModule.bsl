...
Процедура ПрочитатьИОбработатьЗапросБоту() Экспорт
	...
	Если ЗапросБоту.ТипОтвета = Перечисления.ТипыОтветовТелеграм.Кнопка Тогда	
		...
		ИначеЕсли Лев(ДанныеСообщения.callback_query.data, 8) = "УДАЛИТЬ_" Тогда
			
			ИдентификаторЗаказа = Сред(ДанныеСообщения.callback_query.data,9,СтрДлина(ДанныеСообщения.callback_query.data));
			Заказ = НайтиЗаказПоИдентификатору(ИдентификаторЗаказа);
			УдалитьЗаказ(ЗапросБоту, Заказ);
			
		ИначеЕсли Лев(ДанныеСообщения.callback_query.data, 10) = "УДАЛИТЬДА_" Тогда
			
			ИдентификаторЗаказа = Сред(ДанныеСообщения.callback_query.data,11,СтрДлина(ДанныеСообщения.callback_query.data));
			Заказ = НайтиЗаказПоИдентификатору(ИдентификаторЗаказа);
			УдалитьЗаказПослеПодтверждения(ЗапросБоту, Заказ);
			
		ИначеЕсли Лев(ДанныеСообщения.callback_query.data, 11) = "УДАЛИТЬНЕТ_" Тогда
			
			 // получить информацию о заказе и вывести ее
			ИдентификаторЗаказа = Сред(ДанныеСообщения.callback_query.data,12,СтрДлина(ДанныеСообщения.callback_query.data));
			Заказ = НайтиЗаказПоИдентификатору(ИдентификаторЗаказа);
			ТекстСообщения = ПолучитьТекстЗаказа(Заказ, Мастер);
		...
		ИначеЕсли ДанныеСообщения.callback_query.data = "НОМЕНКЛАТУРА" Тогда
			
			ТекстСообщения = ПолучитьТекстУслуги(Мастер);
			Результат = Телеграм.sendMessage(ЗапросБоту.Чат, ТекстСообщения,Перечисления.ВидФорматирования.HTML);
		...
		Если СокрЛП(ДанныеСообщения.message.text) = "/start"  Тогда
			...
			Иначе 
				...
				Иначе
					...
					Если Контрагент.Пустая() Тогда
						ТекстСообщения = "Привіт, " + ДанныеСообщения.message.chat.first_name + "!" + Символы.ПС
						+ "Для початку, повідом свій номер телефону, натиснувши на кнопку"; // + Телеграм.Эмоджи.point_down;
...
Процедура ОбработатьРезультат(ЗапросБоту, ТекстСообщения, Результат)
	...
	Если НЕ Результат = Неопределено Тогда
		...
		Иначе 
			РегистрыСведений.ЗапросыБоту.ЗакрытьСообщение(Телеграм.Бот, ЗапросБоту.update_id);
			ТекстОписания = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку("Бот: [Бот] 
			|Чат: [Чат]
			|Текст: [ТекстСообщения]", Новый Структура("Бот, Чат, ТекстСообщения", ЗапросБоту.Бот, ЗапросБоту.Чат, ЗапросБоту.text));
			ЗаписьЖурналаРегистрации("Телеграм.TestZakazUslugBot", УровеньЖурналаРегистрации.Ошибка,,, ТекстОписания);	
		КонецЕсли;
	КонецЕсли;
...
Процедура УдалитьЗаказ(ЗапросБоту, Заказ)
...
Процедура УдалитьЗаказПослеПодтверждения(ЗапросБоту, Заказ)
...
Функция ОповеститьКонтрагента(ИДКонтрагента, ТекстСообщения) Экспорт 
...
Функция ПолучитьКнопкиМеню()
	...
	КнопкаЗАПИСАТЬСЯ = Телеграм.InlineKeyboardButton(Телеграм.Эмоджи.heavy_plus_sign + " Новий запис", ,"ЗАПИСАТЬСЯ");
	КнопкаМОИЗАПИСИ = Телеграм.InlineKeyboardButton("Мої записи", ,"МОИЗАПИСИ");
	КнопкаНОМЕНКЛАТУРА = Телеграм.InlineKeyboardButton("Послуги/Ціни", ,"НОМЕНКЛАТУРА");
	СтрокаКнопок1 = Телеграм.InlineKeyboardMarkup_add(КнопкаЗАПИСАТЬСЯ, КнопкаМОИЗАПИСИ, КнопкаНОМЕНКЛАТУРА);
...
Функция ПолучитьКнопкиКалендарь(ТекДата, Мастер)
	...
	СтрокаКнопокПодвал1 = Телеграм.InlineKeyboardMarkup_add(КнопкаВыборУслуг);
	Телеграм.InlineKeyboardMarkup_row(МассивСтрок,СтрокаКнопокПодвал1);
	
	СтрокаКнопокПодвал2 = Телеграм.InlineKeyboardMarkup_add(КнопкаУдалитьЗаказ);
	Телеграм.InlineKeyboardMarkup_row(МассивСтрок,СтрокаКнопокПодвал2);
...
Функция ПолучитьКнопкиВремя(ТекДата, Мастер, ДлительностьУслуг)
	...
	тзЗаписей = ПолучитьТаблицуЗаписей(Мастер, ТекДата, ДлительностьУслуг);
	...
	СтрокаКнопок1= Телеграм.InlineKeyboardMarkup_add(КнопкаДругойДень);
	Телеграм.InlineKeyboardMarkup_row(МассивСтрок,СтрокаКнопок1);
	СтрокаКнопок2= Телеграм.InlineKeyboardMarkup_add(КнопкаВыборУслуг);
	Телеграм.InlineKeyboardMarkup_row(МассивСтрок,СтрокаКнопок2);
	СтрокаКнопок3= Телеграм.InlineKeyboardMarkup_add(КнопкаУдалитьЗаказ);
	Телеграм.InlineKeyboardMarkup_row(МассивСтрок,СтрокаКнопок3);
...
Функция ПолучитьТаблицуЗаписей(Знач Мастер, Знач ТекДата, знач ДлительностьУслуг) Экспорт
...
Функция ПолучитьКнопкиРедактироватьЗаказ(Заказ)
	...
	Если ДобавитьВариантОтвета(Заказ.Ссылка.УникальныйИдентификатор(), "СсылкаЗаказ", Заказ.Ссылка) Тогда
		ТекстКнопки = Строка(Телеграм.Эмоджи.x) + " Видалити запис " + Строка(Телеграм.Эмоджи.arrow_up);
		КнопкаУдалитьЗаказ = Телеграм.InlineKeyboardButton(ТекстКнопки, , "УДАЛИТЬ_" + Строка(Заказ.Ссылка.УникальныйИдентификатор()));
	КонецЕсли;

	СтрокаКнопок2 = Телеграм.InlineKeyboardMarkup_add(КнопкаУдалитьЗаказ);
	Телеграм.InlineKeyboardMarkup_row(МассивСтрок,СтрокаКнопок2);
	...
	Если ЭтоДиалогСМастером Тогда
		...
		СтрокаКнопок2 = Телеграм.InlineKeyboardMarkup_add(КнопкаСвободноеВремя); //КнопкаКалендарь
		Телеграм.InlineKeyboardMarkup_row(МассивСтрок,СтрокаКнопок2);
...
Функция ПолучитьКнопкиУдалитьЗаказДаНет()
	...
	Если ДобавитьВариантОтвета(Заказ.Ссылка.УникальныйИдентификатор(), "СсылкаЗаказ", Заказ.Ссылка) Тогда
		КнопкаУдалитьЗаказДа = Телеграм.InlineKeyboardButton(Строка(Телеграм.Эмоджи.whiteheavycheckmark) + " Так", ,"УДАЛИТЬДА_"+Строка(Заказ.Ссылка.УникальныйИдентификатор()));
	КонецЕсли;	
	
	Если ДобавитьВариантОтвета(Заказ.Ссылка.УникальныйИдентификатор(), "СсылкаЗаказ", Заказ.Ссылка) Тогда
		КнопкаУдалитьЗаказНет = Телеграм.InlineKeyboardButton(Строка(Телеграм.Эмоджи.negative_squared_cross_mark) + " Ні", ,"УДАЛИТЬНЕТ_"+Строка(Заказ.Ссылка.УникальныйИдентификатор()));	
	КонецЕсли;
...
Функция ПолучитьКнопкиМенюДляМастера()
	...
	СтрокаКнопок1 = Телеграм.InlineKeyboardMarkup_add(КнопкаСвободноеВремя); //КнопкаКалендарь
	Телеграм.InlineKeyboardMarkup_row(МассивСтрок,СтрокаКнопок1);
...
Функция ПолучитьТекстУслуги(Мастер)
	
	ТекстСообщения = "<b>Послуги:</b>";
	
	ВыборкаУслугиМастера = ПолучитьУслугиМастера(Мастер);
	
	Пока ВыборкаУслугиМастера.Следующий() Цикл
		
		ТекстСообщения = ТекстСообщения + Символы.ПС + ВыборкаУслугиМастера.Наименование + " - " + ВыборкаУслугиМастера.Цена + " грн."; 
...
Функция НайтиЗаказПоИдентификатору(ИдентификаторЗаказа)
	
	Заказ = Документы.ЗаписьНаПрием.ПустаяСсылка();
	
	Если ЗначениеЗаполнено(ИдентификаторЗаказа) Тогда
		мОтветов = НайтиОтвет(ИдентификаторЗаказа);
		стрОтвет = мОтветов[0];
...
Функция СоздатьКонтрагента(ЗапросБоту, Результат, Телефон = Неопределено)
	...
	КонтрагентОбъект.Наименование = Телефон;
...
Функция ПолучитьТекстНеПроведенногоЗаказа(ЗаписьНаДату, Заказ, Мастер = Неопределено)
	...
	мСтрок.Добавить("<b>Запис на " + Формат(ЗаписьНаДату, "ДЛФ=D") + ":</b> (" + Заказ.Номер + ")");
...
Функция ПолучитьТекстЗаказа(Заказ, Мастер) Экспорт 
	...
	Пока ВыборкаМастер.Следующий() Цикл
		...
		Пока ВыборкаЗаказ.Следующий() Цикл
			...
			ВыборкаДатаЗаписи = ВыборкаЗаказ.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, "ДатаЗаписи");
			
			Пока ВыборкаДатаЗаписи.Следующий() Цикл
				
				мСтрок.Добавить("<b>Запис на " + Формат(ВыборкаДатаЗаписи.ДатаЗаписи, "ДЛФ=Д") + ":</b> (" + ВыборкаДатаЗаписи.Номер + ")");
				мСтрок.Добавить("Клієнт: " + ВыборкаДатаЗаписи.КонтрагентНаименование + " (" + ВыборкаДатаЗаписи.Телефон + ")");
				мСтрок.Добавить("Майстер: " + Мастер);
				
				ВсегоСумма = 0;
				
				ВыборкаДетальныеЗаписи = ВыборкаДатаЗаписи.Выбрать();
...
Функция ПолучитьОписаниеЗаказа(ИДКонтрагента, Результат)
	...
	Пока ВыборкаЗаказы.Следующий() Цикл
		...
		мСтрок.Добавить("<b>Запис на " + Формат(ВыборкаЗаказы.ДатаЗаписи, "ДЛФ=D") + " (" + ВыборкаЗаказы.РегистраторНомер + "):</b>");
