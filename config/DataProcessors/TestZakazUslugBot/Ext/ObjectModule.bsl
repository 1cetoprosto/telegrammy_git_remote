...
// to-do:
// 10.03.19 убрать передачу переменной Сообщение, так как она стала глобальной в обработке
// 13.03.19 Рефакторинг работы с СессияКонтрагента (в конце процедуры записываем сессию, в коде вставляем переменные в структуру) 
////////////////////////////////////////////////////////////////////////////////
...
Процедура ПрочитатьИОбработатьСообщение(Сообщение) Экспорт
	...
	ДанныеСообщения = Сообщение.ДанныеЗапроса.Получить();
	...
	//ЭтоДиалогСМастером = ЭтоМастер(Сообщение);
	ЭтоДиалогСМастером = Ложь;
	ИдентификаторМастера = Неопределено;
	ОписаниеМастера = ПолучитьМастераПоБоту(Телеграм.Бот);
	ОписаниеМастера.Свойство("Идентификатор", ИдентификаторМастера);
	
	Если ИдентификаторМастера <> Неопределено И ИдентификаторМастера = Сообщение.Чат Тогда
	...
	Если Сообщение.ТипОтвета = Перечисления.ТипыОтветовТелеграм.Контакт Тогда 
		...
		// проверим логин
		Телефон = Прав(СокрЛП(Сообщение.text),10);
		
		Если АктивацияБота Тогда
			
			Мастер = Справочники.Мастера.НайтиПоРеквизиту("Телефон", Телефон);	
			Если НЕ Мастер.Пустая() Тогда
				Если ЗаписатьИДТелеграм(Мастер, Сообщение.Чат) Тогда
					ТекстСообщения = "Активація бота пройшла успішно";
					Результат = Телеграм.sendMessage(Сообщение.Чат, ТекстСообщения,,,,,ПолучитьКнопкиМенюДляМастера()); //Телеграм.ReplyKeyboardRemove()
				...
				КонецЕсли;
			Иначе
				ТекстСообщения = "Ми не знайшли Бота зареэстрованого на Ваш номер: " +СокрЛП(Сообщение.text)+ Символы.ПС
		...
		Иначе 
			...
			Если Контрагент.Пустая() Тогда
				Контрагент = СоздатьКонтрагента(Сообщение, Результат, Телефон);
				...
				//Результат = Телеграм.editMessageText(Сообщение.Чат, Сообщение.message_id,, ТекстСообщения,,,ПолучитьКнопкиГлавногоМеню());
			Иначе
				Если ЗаписатьИДТелеграм(Контрагент, Сообщение.Чат) Тогда
					ТекстСообщения = "Реєстрація пройшла успішно" + Символы.ПС
					+ "Для запису, натисни кнопку ""Новий запис"" ";
					Результат = Телеграм.sendMessage(Сообщение.Чат, ТекстСообщения,,,,,ПолучитьКнопкиМеню());
					//Результат = Телеграм.editMessageText(Сообщение.Чат, Сообщение.message_id,, ТекстСообщения,,,ПолучитьКнопкиГлавногоМеню());
	...
	#Область КнопкиСтарыеДляМастера
	...
	Если Сообщение.ТипОтвета = Перечисления.ТипыОтветовТелеграм.Кнопка Тогда	
		...
		ИначеЕсли ДанныеСообщения.callback_query.data = "ЗАПИСАТЬСЯ" Тогда
			...
			Если ЭтоДиалогСМастером Тогда
				...
				ПоказатьЗапросТелефонаКонтрагента(Сообщение);
			Иначе
				
				Контрагент = Справочники.Контрагенты.НайтиПоРеквизиту("Идентификатор", Сообщение.Чат);
				Если Контрагент.Пустая() Тогда
					Контрагент = СоздатьКонтрагента(Сообщение, Результат);
				КонецЕсли;
	...
	ИначеЕсли Сообщение.ТипОтвета = Перечисления.ТипыОтветовТелеграм.Текст Тогда	
		...
		Если СокрЛП(ДанныеСообщения.message.text) = "/start"  Тогда
			...
			Если ИдентификаторМастера = 0 Тогда
				ТекстСообщения = "Привіт, " + ДанныеСообщения.message.chat.first_name + "! Бот ще не активований." + Символы.ПС
				+ "Введи код активації.";
				Результат = Телеграм.sendMessage(Сообщение.Чат, ТекстСообщения);
			Иначе 
				Если ЭтоДиалогСМастером Тогда
					ТекстСообщения = "Привіт, " + ДанныеСообщения.message.chat.first_name + "! Радий тобі.";
					Результат = Телеграм.sendMessage(Сообщение.Чат, ТекстСообщения,,,,,ПолучитьКнопкиМенюДляМастера());
				Иначе
					Контрагент = Справочники.Контрагенты.НайтиПоРеквизиту("Идентификатор", Сообщение.Чат);
					Если Контрагент.Пустая() Тогда
...
Процедура ПоказатьВыборУслуг(Сообщение, стрОтвет = Неопределено)
	...
	//	ЗафиксироватьОшибку(Сообщение, ТекстСообщения);
	//	Возврат;
	//КонецЕсли;
	...
	Иначе
		Если мНоменклатура.Количество() > 0 Тогда // есть выбранная номенклатура
			Результат = Телеграм.editMessageText(Сообщение.Чат, Сообщение.message_id,, ТекстСообщения,Перечисления.ВидФорматирования.HTML,,ПолучитьКнопкиУслуги(Мастер, мНоменклатура));
		...
		Иначе //Если ПредыдущийСообщение.Свойство("ТекДата", ТекДата) Тогда
			//Результат = Телеграм.editMessageText(Сообщение.Чат, Сообщение.message_id,, ТекстСообщения,Перечисления.ВидФорматирования.HTML,,ПолучитьКнопкиУслуги(Мастер));
...
Процедура УдалитьЗаказ(Сообщение)
	...
	Иначе 
		Если ЭтоДиалогСМастером Тогда
			...
			//Результат = Телеграм.sendMessage(Сообщение.Чат, ТекстСообщения,,,,,ПолучитьКнопкиМенюДляМастера());
			Результат = Телеграм.editMessageText(Сообщение.Чат, Сообщение.message_id,, ТекстСообщения,Перечисления.ВидФорматирования.HTML,,ПолучитьКнопкиМенюДляМастера());
		Иначе 
			ТекстСообщения = "Привіт, " + ДанныеСообщения.callback_query.message.chat.first_name + "!" + Символы.ПС + 
			"Для запису, натисни кнопку ""Новий запис"" "; // + Телеграм.Эмоджи.point_down;
			//Результат = Телеграм.sendMessage(Сообщение.Чат, ТекстСообщения,,,,,ПолучитьКнопкиМеню());
			Результат = Телеграм.editMessageText(Сообщение.Чат, Сообщение.message_id,, ТекстСообщения,Перечисления.ВидФорматирования.HTML,,ПолучитьКнопкиМеню());
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаписатьВремя(Сообщение)
	...
	Попытка
		...
		//КонецЦикла;
	Исключение
		ЗафиксироватьОшибку(Сообщение, "Помилка збереження запису! Звернись в підтримку за тел. (067) 260-31-07");
		Возврат;
	КонецПопытки;
	
	ТекстСообщения = ПолучитьТекстЗаказа(Заказ, Мастер);
	Результат = Телеграм.editMessageText(Сообщение.Чат, Сообщение.message_id,, ТекстСообщения,Перечисления.ВидФорматирования.HTML,, ПолучитьКнопкиРедактироватьЗаказ(Заказ));
...
Процедура ПоказатьЗапросТелефонаКонтрагента(Сообщение)
...
Функция СоздатьКонтрагента(Сообщение, Результат, Телефон = Неопределено)
	
	КонтрагентОбъект = Справочники.Контрагенты.СоздатьЭлемент();
	//КонтрагентОбъект.Заполнить(Сообщение);
	КонтрагентОбъект.Телефон = Телефон;
	Если Не ЭтоДиалогСМастером Тогда
		КонтрагентОбъект.Идентификатор = Сообщение.Чат;
		КонтрагентОбъект.ТелеграмИмя =  Сообщение.first_name;
		КонтрагентОбъект.Наименование = Сообщение.first_name;
	КонецЕсли;
