...
Процедура ПрочитатьИОбработатьСообщение(Сообщение) Экспорт
	...
	Если Сообщение.ТипОтвета = Перечисления.ТипыОтветовТелеграм.Кнопка Тогда	
		...
		ИначеЕсли Лев(ДанныеСообщения.callback_query.data,14) = "ДЕНЬКАЛЕНДАРЯ_" Тогда
			...
			//ВыборкаМастеров = ПолучитьМастеров(Телеграм.Бот);
			//Если ВыборкаМастеров.Количество() = 0 Тогда
			//	ЗаписьЖурналаРегистрации("Телеграм.ПолучитьМастеров",УровеньЖурналаРегистрации.Предупреждение,,,"Мастрів не знайдено: " + Телеграм.Бот);
			//ИначеЕсли ВыборкаМастеров.Количество() = 1 Тогда
			//	// один мастер
			//	ВыборкаМастеров.Следующий();
			//	Мастер = ВыборкаМастеров.Мастер;
			//	Боты.СоздатьСессиюКонтрагента(Телеграм.Бот, Сообщение.Чат, Новый Структура("Мастер", Мастер));
			
			// ищем заказ
			Заказ = НайтиЗаказ(Сообщение.Чат, Результат); //, ЗаписьНаДату
			...
			ТекстСообщения = "Виберіть послуги, а потім час:";
			//Результат = Телеграм.sendMessage(Сообщение.Чат, ТекстСообщения,,,,,ПолучитьКнопкиКалендарь(ТекущаяДата(), Мастер));
			...
			//Иначе
			//	// много мастеров
			//	ТекстСообщения = "Виберіть майстра:";
			//	Результат = Телеграм.sendMessage(Сообщение.Чат, ТекстСообщения,,,,,ПолучитьКнопкиМастера(Телеграм.Бот));
			//КонецЕсли;
		...
		ИначеЕсли Лев(ДанныеСообщения.callback_query.data,6) = "ВРЕМЯ_" Тогда
			...
			Исключение
				...
				ТекстСообщения = "Помилка запису! Спробуйте ще раз.";
			КонецПопытки;
		...
		ИначеЕсли ДанныеСообщения.callback_query.data = "ЗАПИСАТЬСЯ" Тогда
			...
			//Заказ = НайтиЗаказ(Сообщение.Чат, Результат);
			//Если Заказ = Неопределено Тогда
			//	ТекстСообщения = "Ваш запис не знайдено!";
			//	Результат = Телеграм.sendMessage(Сообщение.Чат, ТекстСообщения);
			...
			Если ПредыдущийСообщение.Свойство("Мастер", Мастер) Тогда
				...
				ТекстСообщения = "Виберіть дату:";
				//Результат = Телеграм.sendMessage(Сообщение.Чат, ТекстСообщения,,,,,ПолучитьКнопкиКалендарь(ТекущаяДата(), Мастер));
				Результат = Телеграм.editMessageText(Сообщение.Чат, Сообщение.message_id,, ТекстСообщения,,,ПолучитьКнопкиКалендарь(ТекущаяДата(), Мастер));
				
				ПредыдущийСообщение.Вставить("ТекДата", ТекущаяДата());
				Боты.СоздатьСессиюКонтрагента(Телеграм.Бот, Сообщение.Чат, ПредыдущийСообщение);
				
			КонецЕсли;
			
		ИначеЕсли ДанныеСообщения.callback_query.data = "ВЫБРАТЬВРЕМЯ" Тогда
			...
			Мастер = Неопределено; ЗаписьНаДату = "";
			ПредыдущийСообщение = Боты.ПолучитьОписаниеСессииКонтрагента(Телеграм.Бот, Сообщение.Чат);
			ПредыдущийСообщение.Свойство("Мастер", Мастер);
			ПредыдущийСообщение.Свойство("ТекДата", ЗаписьНаДату);
...
Функция ПолучитьКнопкиУслуги(Мастер, мУслуг = Неопределено)
	...
	Пока ВыборкаУслугиМастера.Следующий() Цикл
		Если ДобавитьВариантОтвета(ВыборкаУслугиМастера.Ссылка.УникальныйИдентификатор(), "ВыборУслуги", ВыборкаУслугиМастера.Ссылка) Тогда
			...
			Если (НЕ мУслуг = Неопределено) И (мУслуг.Количество() > 0) Тогда
	...
	КнопкаВыбратьВремя = Телеграм.InlineKeyboardButton("Вибрати час",, "ВЫБРАТЬВРЕМЯ");
	СтрокаКнопок2 = Телеграм.InlineKeyboardMarkup_add(КнопкаВыбратьВремя);
...
Функция ПолучитьКнопкиКалендарь(ТекДата, Мастер)
	...
	СтрокаКнопок1 = Телеграм.InlineKeyboardMarkup_add(КнопкаМесяцГод);
	Телеграм.InlineKeyboardMarkup_row(МассивСтрок,СтрокаКнопок1);
	...
	Кнопка = Телеграм.InlineKeyboardButton(" ", , "ИГНОР");
	КнопкаВперед = Телеграм.InlineKeyboardButton(">", , "СЛЕДУЮЩИЙМЕСЯЦ");
	
	СтрокаКнопокВпередНазад = Телеграм.InlineKeyboardMarkup_add(КнопкаНазад, Кнопка, КнопкаВперед);
...
Функция ПолучитьКнопкиВремя(ТекДата, Мастер, ДлительностьУслуг)
	...
	КнопкаДругойДень = Телеграм.InlineKeyboardButton("Інший день", ,"ЗАПИСАТЬСЯ");
	СтрокаКнопок = Новый Массив;
...
//{[+](фрагмент ДОБАВЛЕН), Admin 16.12.2018 16:09:30
// сделано для проверки помещения в хранилище, а потом на github
А = 1;
А = А + 1;
// второй комит по первой задаче
С = 2;
С = С + 2;
//}Admin 16.12.2018 16:09:30
