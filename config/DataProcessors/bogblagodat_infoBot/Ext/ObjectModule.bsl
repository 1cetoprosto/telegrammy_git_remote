...
// Параметры (название, тип, дифференцированное значение)
//
Функция Объявления(ЗапросБоту=Неопределено) Экспорт 
	...
	Результат = Телеграм.sendMessage(?(ЗапросБоту=Неопределено,"-1001227968714",ЗапросБоту.Чат), 
	ПараметрыСообщения.text,
	ПараметрыСообщения.ВидФорматирования,
	ПараметрыСообщения.ОтключитьПредварительныйПросмотрСсылок,
	ПараметрыСообщения.ТихаяОтправка);
...
Процедура ПрочитатьИОбработатьЗапросБоту() Экспорт
	...
	Если ЗапросБоту.ТипОтвета = Перечисления.ТипыОтветовТелеграм.Кнопка  Тогда	
		...
		Если ДанныеСообщения.callback_query.data = "ОБЪЯВЛЕНИЯ" Тогда
			...
			Результат = Объявления(ЗапросБоту);
			
		ИначеЕсли ДанныеСообщения.callback_query.data = "ПОДКРЕПИСЬ" Тогда
			
			ТекстСообщения = "Виберіть:";
			
			Результат = Телеграм.sendMessage(ЗапросБоту.Чат, ТекстСообщения,,,,,ПолучитьКнопкиПодкрепись());
			
		ИначеЕсли ДанныеСообщения.callback_query.data = "ОНАС" Тогда
			
			ТекстСообщения = "Виберіть:";
			
			Результат = Телеграм.sendMessage(ЗапросБоту.Чат, ТекстСообщения,,,,,ПолучитьКнопкиОНас());
			
		ИначеЕсли ДанныеСообщения.callback_query.data = "МЫНАКАРТЕ" Тогда
			
			Результат = Телеграм.sendLocation(ЗапросБоту.Чат, 48.6777, 26.6113); // .sendMessage(ЗапросБоту.Чат, ТекстСообщения,,,,,ПолучитьКнопкиОНас());
		...
		//	ИдентификаторАкцепта = Боты.СоздатьСессию(Бот, Новый Структура("Действие, Контрагент", "Акцепт", ЗапросБоту.Контрагент));
		//	ИдентификаторЗапрета = Боты.СоздатьСессию(Бот, Новый Структура("Действие, Контрагент", "Запрет", ЗапросБоту.Контрагент));
		...
		//	parse_mode = "Markdown";
		//	Телеграм.sendMessage(Выборка.Контрагент.ТелеграмID, СтрШаблон("Поступил запрос на предоставление доступа от [%1](http://t.me/%2).", ЗапросБоту.Контрагент.ТелеграмИмя, ЗапросБоту.Контрагент.ТелеграмНик), parse_mode,,,, reply_markup);
		//КонецЦикла;
	...
	Если НЕ Результат = Неопределено Тогда
		...
		Иначе 
			ТекстОписания = СтрШаблон("%1
			|Чат: %2
			|Текст: %3", Результат.Получить("description"), ЗапросБоту.Чат, ТекстСообщения);
			ЗаписьЖурналаРегистрации("ИнфоБот", УровеньЖурналаРегистрации.Ошибка,,, ТекстОписания);
		КонецЕсли;
	КонецЕсли;

	Если ЗапросБоту.ТипОтвета = Перечисления.ТипыОтветовТелеграм.ПустаяСсылка() Тогда
		// пришло неизвестно что
		Боты.ЗакрытьСообщение(Телеграм.Бот, ЗапросБоту.update_id);
		ТекстОписания = СтрШаблон("Чат: %1
			|Текст: %2", ЗапросБоту.Чат, ТекстСообщения);
		ЗаписьЖурналаРегистрации("ИнфоБот", УровеньЖурналаРегистрации.Ошибка,,, ТекстОписания);
	КонецЕсли;
